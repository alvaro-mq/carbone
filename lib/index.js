...
function walkFiles (template, data, options, currentIndex, callback) {
  ...
  if (_file.isMarked===true) {
    builder.buildXML(_file.data, data, options, function (err, xmlResult, allImages) {
      if (err) {
        return callback(err, template);
      }
      // replace images with array buffer of data
      if (allImages) {
        for (let image of allImages) {
          for (let file of template.files) {
            if (image.picture === file.name && data[image.name]) {
              file.data = Buffer.from(data[image.name], 'base64');
              break;
            }
          }
        }
