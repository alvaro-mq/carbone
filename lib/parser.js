...
var parser = {
   ...
   * All images are declared with braces like this: <draw:frame.
   * It returns an array of objects which contains two things:
   * - name of picture
   * - name of marker
   * @param {string} xml : xml string
   * @param {Function} callback(err, allImages) where "allImages" is an array like this [{name: 'imgExample', picture:'Picture/1234.jpg'}]
   */
  findImages : function (xml, callback) {
    var _allImages = [];
    var _cleanedXml = xml;
    _cleanedXml.replace(/<draw:frame(.+?)<\/draw:frame>/g, function (i, image) {
      const indexStartPicture = image.indexOf('Picture');
      const indexEndPicture = image.substr(indexStartPicture).indexOf('"');
      const picture = image.substr(indexStartPicture, indexEndPicture);
      const indexStartMarker = image.indexOf('{d.') + 3;
      const indexEndMarker = image.substr(indexStartMarker).indexOf('}');
      const name = image.substr(indexStartMarker, indexEndMarker);
      const _obj = {
        name,
        picture,
      };
      _allImages.push(_obj);
      return i;
    });
    process.nextTick(function () {
      callback(null, _allImages);
    });
  },
  /**
